...
1. In the dropdown menu click **Web App**.
...
1. Select **Next: Database >**. *(Note that this tab may not be available in your lab subscription.)*
...
1. On the **Container** tab, perform the following actions, and then select **Review + create**:
    ...
    | Setting | Action |
    | -- | -- |
    | **Image Source** drop-down list | Select **Other container registries** |
...
1. On the blade displaying the newly created app properties, wait until the full page has loaded then select **Browse**.
...
1. Switch back to the browser window that displays the Azure Portal page for the **httpapi**_[yourname]_ web app.
...
1. In the **Design** section, on the **Inbound processing** tile select **+ Add policy**. *(You may need to scroll down to see the Add link.)*
...
1. Back on the **Design** tab, in the list of operations, select **Echo Headers**, and then select the **Test** tab. *(Note: If you only see scrolling dots, click "Echo Headers" again.)*
...
1. Review the results of the API request. *(Scroll down to see the results.)*
...
1. Select **Test**. *(Note: If you only see scrolling dots, click "Get Legacy Data" again.)*
...
1. On the **Design** pane, in the **Outbound processing** section, select **Add policy**. *(You may need to scroll down to see the Add link.)*
...
1. Back on the **Design** tab, in the list of operations, select **Get Legacy Data**, and then select **Test**. *(Note: If you only see scrolling dots, click "Get Legacy Data" again.)*
...
1. Within the **HTTP response** section, perform the following actions:
    ...
    1. Select the **Trace** tab in the **HTTP response** secion, review the content in the **Backend** and **Outbound** text boxes (you will need to scroll down to find these), and note that they include details of the corresponding API operations with their timing information.
...
1. Back on the **Design** tab, in the list of operations, select **Modify Status Code**, and then select the **Test** tab. *(Note: If you only see scrolling dots, click "Modify Status Code" again.)*
